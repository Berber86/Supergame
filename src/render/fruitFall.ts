...
export function drawFruitFall(d: DrawCtx, world: World): void {
  ...
  const fromX = (topX + s.x) * sc * mirror,
    fromY = (-geom.height + s.y + 7 * geom.scale) * sc;
  ...
  const tile = world.at(Math.floor(land.x), Math.floor(land.y)),
    r = world.grow?.rect;
  if (!tile || tile.water || tile.indoor || tile.veranda || !['grass', 'moss', 'soil'].includes(tile.ground)) return;
