 ...
 *
 * Расширение — награда за деятельность, а не покупка: совершил два
 * действия — сад предлагает вырасти вдвое вдоль одной из сторон
 * (четыре зоны-кандидата, туман приподнимается, видно, что там:
 * вода, грива, роща). Дальше порог удваивается: 4, 8, 16 действий.
 *
...
export interface GrowState {
  ...
  progress: number;
  /** Сколько расширений было: порог следующего = 2 << stage. */
  stage: number;
...
/** Запас не растёт бесконечно: шесть действий впрок — час отсутствия. */
...
/**
 * Пороги роста — баланс растущего сада.
 *
 * Было 1,2,4,8,16,32,64 — экспонента убивала позднюю игру: последнее
 * расширение требовало 64 действия = 10ч40м чистого ожидания.
 * Стало мягче: 1,2,4,7,11,16,22 — ранняя игра всё так же быстрая,
 * поздняя — 22 действия = 3ч40м, всего до полного 16×16 нужно 63 действия
 * ≈ 10.5 часов распределённого времени вместо 21 часа.
 */
const GROW_THRESHOLDS = [1, 2, 4, 7, 11, 16, 22] as const;

export function growThreshold(stage: number): number {
  if (stage < 0) return 1;
  if (stage < GROW_THRESHOLDS.length) return GROW_THRESHOLDS[stage];
  // за пределами — растёт по +8, чтобы не запереть кастомные стадии
  return GROW_THRESHOLDS[GROW_THRESHOLDS.length - 1] + (stage - GROW_THRESHOLDS.length + 1) * 8;
}
