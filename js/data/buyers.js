/**
 * buyers.js — точки сбыта. ТОЛЬКО ДАННЫЕ; логика продажи — js/core/trade.js
 * (сессия 7), продажа вслепую — js/core/identify.js (сессия 6).
 *
 * accepts:        список категорий предметов (см. items.js) или 'all'
 * acceptsUnidentified: берёт ли неопознанное (🧺 — «кот в мешке» по твоей
 *                 оценке; 🕶️ — вслепую, особый азартный редьюсер)
 * priceMult:      коэффициент к цене предмета (item.value или истинной цене)
 * tradeBonus:     прибавка на 🤝 Торге-10 (по уровням линейно, GDD §8)
 * scam:           параметры кидка (только для перекупщика; формула честности —
 *                 trade.js → perekupHonestChance, потому что азарт один)
 */
